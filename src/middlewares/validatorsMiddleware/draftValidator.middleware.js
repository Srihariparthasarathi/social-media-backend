...
const createDraftValidator = async (req, res, next)=>{
    const rules = [
        body('caption')
        ...
        body('image')
        .custom((value, {req}) =>{
           if(!req.file) throw new Error("Oops Image require to draft Post, Please try again with uploding an image");
           return true;
        })
...
const updateDraftValidator = async (req, res, next)=>{
    const rules = [
        body('caption')
        .optional()
        .isLength({ min: 5, max: 2200 })
        ...
        .optional()
        .custom((value, {req}) =>{
           if(!req.file) throw new Error("Oops Image require to Update Draft item, Please try again with uploding an image");
           return true;
        })
...
export { createDraftValidator, updateDraftValidator };